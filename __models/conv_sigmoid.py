...
if __name__ == "__main__":
    ...
    model = CS(n_channels=3, n_classes=2, bilinear=True).cuda().eval() 
    outputs = model(inputs)

    from __debug import summary, quant_nn, export_onnx_quant
    summary(model, input_dims=(3, 640, 640)) 

    model = CS(n_channels=3, n_classes=2, bilinear=True, quantize=True).cuda().eval() 
    import os
