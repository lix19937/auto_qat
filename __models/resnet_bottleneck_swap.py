...
# based resnet_bottleneck.py, just swap shortcut branch quant to another side
...
class Bottleneck(torch.nn.Module):
    ...
    def forward(self, x):
        if self.quantize:
            y = self.shortcut(x) + self.residual_quantizer(self.conv(x))
        else:
...
if __name__ == "__main__":
    ...
    summary(model, input_dims=(3, 224, 224)) 

    model = ResNet50(num_classes=1000, sz = 224, quantize=True).cuda().eval() 
    import os
