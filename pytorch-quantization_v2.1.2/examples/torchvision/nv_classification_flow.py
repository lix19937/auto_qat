...
import sys  
sys.path.insert(0, "/home/igs/Desktop/cudaacc/share/auto_qat/pytorch-quantization_v2.1.2")  

'''
cd ./pytorch-quantization/examples/torchvision
...
--sensitivity \
--num-calib-batch=1 \  
--ckpt-path=./deps/resnet50-19c8e357.pth \
--evaluate-onnx  \
...
sys.path.insert(0,"/home/igs/Desktop/cudaacc/share/auto_qat/pytorch-quantization_v2.1.2/examples/torchvision/deps/vision-0.9.0/references/classification/")
...
def prepare_model(
    ...
    if model_name in models.__dict__:
        logger.debug("use manual q-dq, pretrained:", pretrained)
        model = models.__dict__[model_name](pretrained=pretrained, quantize=True)
    else:
        logger.debug("use auto q-dq")
    ...
    if not pretrained:
        ...
        logger.debug("load state dict")    
        logger.info(model);exit(0)
...
def main(cmdline_args):
    ...
    dump_model(model, "after_calib_nnmodule.log")
    
    ## Evaluate after calibration
    if args.num_calib_batch > 0:
        with torch.no_grad():
            logger.debug('Calibration evaluation:')
...
def export_onnx(model, onnx_filename, batch_onnx, per_channel_quantization):
    ...
    print("Creating ONNX file: " + onnx_filename, "per_channel_quantization:" , per_channel_quantization)
    dummy_input = torch.ones(batch_onnx, 3, 224, 224, device='cuda') #TODO: switch input dims by model
    try:
    ...
    dump_onnx(onnx_filename, "onnx_dump.log")
    return True
    ...
    for name, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer):
            if module._calibrator is not None:
                ...
                logger.warning("{},{}".format(name, module._calibrator))
                module.disable()

    # Feed data to the network for collecting stats
    for i, (image, _) in tqdm(enumerate(data_loader), total=num_batches):
        model(image.cuda()) ### invoke forward 
        if i >= num_batches:
            break
...
## q-scale of nnmodule 
def dump_model(model:torch.nn.Module, out_name):
  ...
  with open(out_name + ".txt", "w") as f:
    for k,v in model_dict.items():
      if "._amax" in k:
        ...
        flg = ','
        b = [str(i) for i in v.shape] 
        shape_str = '\t[' + flg.join(b) + ']'
        f.writelines(shape_str)
        f.writelines("\n")

        x = v.cpu().detach().numpy().reshape(1, -1).squeeze()/127.
        x = ' '.join(map(str, x.ravel().tolist()))
        f.writelines(x)        
        f.writelines("\n")


## Change Constant back to Initializer through onnxsim
## onnxsim input.onnx output.onnx --skip-shape-inference
def dump_onnx(onnx_file, out_name):
  import onnx
  ...
  nodes = graph.node
  ## store quant_node_name:q-scale np value 
  quant_nodes_scale = {}
  ## store scale_node_name:quant_node_name  
  quant_nodes_name = {}
  ## y_scale/zero point as input with category: Constant
  scale_is_const_input = False

  for node in nodes:
      if node.op_type == "QuantizeLinear":
        assert len(node.input) == 3, 'node.input len is:' + str(len(node.input))
        quant_nodes_name[node.input[1]] = node.name

  for node in nodes:
    if node.op_type == 'Constant':
      assert len(node.output) == 1, 'node.output len is:' + str(len(node.output))
      ## node.output as another node's input  
      scale_node_name = node.output[0]
      if scale_node_name in quant_nodes_name:
        scale_is_const_input = True
        assert len(node.attribute) >= 1, 'node.attribute len is:' + str(len(node.attribute))
        # print(quant_nodes_name[scale_node_name], node.output)
        np_data = numpy_helper.to_array(node.attribute[0].t)
        quant_node_name = quant_nodes_name[scale_node_name]
        quant_nodes_scale[quant_node_name] = np_data

  if scale_is_const_input:
    assert len(quant_nodes_scale) == len(quant_nodes_name), "#1 len(quant_nodes_scale) != len(quant_nodes_name)"
  
  if len(quant_nodes_scale) == 0:
    for init in initializers:
      scale_node_name = init.name
      if scale_node_name in quant_nodes_name:
        np_data = numpy_helper.to_array(init) 
        quant_node_name = quant_nodes_name[scale_node_name]
        quant_nodes_scale[quant_node_name] = np_data

  assert len(quant_nodes_scale) == len(quant_nodes_name), "#2 len(quant_nodes_scale) != len(quant_nodes_name)"

  with open(out_name + ".txt", "w") as f:
    for k, v in quant_nodes_scale.items():
      f.writelines(k)
...
def convert_1():
  grid_size = torch.tensor([2048, 1024, 28]).numpy()
  parse_shape = grid_size[::-1] + [1, 0, 0]
  print(parse_shape);exit(0) # [  29 1024 2048]
